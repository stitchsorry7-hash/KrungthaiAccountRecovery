...
class MainActivity : AppCompatActivity() {
    ...
    private fun analyzeUri(uri: android.net.Uri) { status.text="กำลัง OCR..."; thread { try { val text=ocr.process(uri); show(text, uri.toString()) } catch(e:Exception){ runOnUiThread{status.text="อ่านไฟล์ไม่สำเร็จ: ${e.message}"} } } }
